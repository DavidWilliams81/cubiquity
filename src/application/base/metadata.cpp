...
const Material Metadata::EmptySpace = { "_EmptySpace", { 0.0f, 0.0f, 0.0f} }; // FIXME - Should we actually add a base_color color for this? Or just an opacity of zero?
...
Metadata loadMetadataForVolume(const std::filesystem::path& volumePath)
{
	...
	for (int i = 0; i < metadataAsJSON["materials"].size(); i++)
	{
		...
		metadata.materials[i].base_color =
			metadataAsJSON["materials"][i]["base_color"].template get<std::array<float, 3>>();
	}
...
void saveMetadataForVolume(const Metadata& metadata, const std::filesystem::path& volumePath)
{
	...
	for (int i = 0; i < metadata.materials.size(); i++)
	{
		...
		metadataAsJSON["materials"][i]["base_color"] = metadata.materials[i].base_color;
	}
